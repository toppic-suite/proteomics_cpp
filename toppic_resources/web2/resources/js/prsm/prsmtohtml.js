...
function createTableElements(){
	...
	let l_scans = prsm_data.prsm.ms.ms_header.scans ;
	...
	if(prsm.annotated_protein.annotation.hasOwnProperty('ptm'))
	{
		...
		if(Array.isArray(ptm))
		{
			ptm.forEach(function(ptm, index){
				...
				{
					variable_ptm = variable_ptm + getVariablePtm(ptm) + "] " ;
				}
				if(ptm.ptm_type == "Fixed")
				{
					fixed_ptm = fixed_ptm + getFixedPtm(ptm) + "] " ;
				}
			})
		}
		else
...
function buttonsAndAlerts(para,prsm,id)
{
	...
	d3.select('#resize').on("click", function(){
		...
		if(document.getElementById("show-num").checked)
		{
			para.show_num = true ;
		}
		else
		...
		if(document.getElementById("show-skipped-lines").checked)
		{
