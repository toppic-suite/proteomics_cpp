...
{
  let x = para.left_margin + para.numerical_width + (para.row_length ) * para.letter_width;
  //buffer width to make left and right numbers symmetrical as left numbers are left aligned 
  x = x + ((para.row_length/ para.block_length) - 1) * para.gap_width + para.numerical_width ; 
  let y = getY(para, position, start_value);
  return [x,y];
}
